...
export function playDiscoveryChime(objectClass) {
  try {
    const ctx = getAudioContext();
    if (ctx.state === 'suspended') ctx.resume();
    ...
    if (objectClass === 'bottle') {
      ...
      osc.frequency.linearRampToValueAtTime(880.0, ctx.currentTime + 0.3);
    } else if (objectClass === 'book') {
      osc.type = 'triangle';
      osc.frequency.setValueAtTime(220.0, ctx.currentTime);
      osc.frequency.exponentialRampToValueAtTime(440.0, ctx.currentTime + 0.6);
    } else if (objectClass === 'cell phone' || objectClass === 'phone') {
      osc.type = 'square';
      osc.frequency.setValueAtTime(880.0, ctx.currentTime);
      osc.frequency.setValueAtTime(1760.0, ctx.currentTime + 0.1);
